...
const UserConf = (props: User) =>{
    ...
    const banUser = async (id) => {
        console.log('ban');
        await axios.get(`http://localhost:9000/users/ban/${props.value.id}/${props.value.banned ? 'unban' : 'ban'}`, {headers: {
    ...
    return (
        <div className='user-container'>
            <div className='user-container-data'>
                ...
                { props.value.banned ? <p>User has ban</p> : ''}
                <button className='common-button' onClick={getHistory}>History</button>
                <button className='common-button' onClick={()=>banUser(props.value.id)}>{props.value.banned ? 'Unban user' : 'Ban user'}</button>
            </div>
